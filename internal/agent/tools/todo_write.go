...
//	带了 TodoTool：LLM 在执行动作前，必须先调用这个工具。
//	工具返回的 Output（“还有 3 个任务未完成！”）像复读机一样钉在上下文里。
//	这会强制 LLM 的下一轮预测进入“检索模式”而不是“总结模式”。
//
//	2. 它是 AI 的“外置短期记忆”
...
// Execute 执行待办事项写入工具，创建任务计划并记录执行步骤
//
// 功能说明:
//   - 解析并验证输入参数，提取任务描述和执行步骤
//   - 支持任务描述的默认值（未提供时使用"未提供任务描述"）
//   - 将任务和步骤格式化为可读的执行计划
//   - 返回结构化的计划数据，供Agent跟踪任务进度
//
// 参数:
//   - ctx: 上下文对象（当前未使用，保留用于未来扩展）
//   - args: JSON格式的原始参数，包含以下字段：
//     - Task: string 任务描述（可选），描述本次计划要完成的总体目标
//     - Steps: []string 执行步骤列表（可选），具体的行动步骤
//
// 返回值:
//   - *types.ToolResult: 工具执行结果，包含以下内容：
//     - Success: 是否成功创建计划（固定为true，本工具无失败场景）
//     - Output: 格式化的计划文本（Markdown格式），包含任务标题和步骤列表
//     - Data: 结构化数据（map[string]interface{}），包含：
//         * task: string 任务描述
//         * steps: []string 执行步骤数组
//         * steps_json: string 步骤的JSON序列化字符串
//         * total_steps: int 步骤总数
//         * plan_created: bool 计划是否已创建（固定为true）
//         * display_type: string 展示类型（固定为"plan"）
//     - Error: 错误信息（本工具无错误场景，固定为空）
//   - error: 工具框架层面的错误（参数解析失败时返回）
//
// 执行流程:
//   1. 解析JSON输入参数
//   2. 验证Task字段，为空时设置默认值"未提供任务描述"
//   3. 提取Steps步骤列表（可为空）
//   4. 调用generatePlanOutput生成格式化输出
//   5. 将步骤序列化为JSON字符串
//   6. 组装返回结果
//
// 计划生成:
//   - 通过generatePlanOutput函数格式化输出
//   - 典型格式：
//     ## 任务计划: {Task}
//
//     总步骤数: {n}
...
//
// 数据用途:
//   - steps_json: 便于前端直接解析步骤数据，无需二次序列化
//   - display_type: 前端根据此字段渲染专门的计划展示组件
//   - plan_created: 标记计划状态，支持后续的进度更新
//
// 错误处理:
//   - 参数解析失败: 返回解析错误，err非nil
//   - 其他场景: 无错误，Task有默认值，Steps可为空
//
// 使用场景:
//   - Agent开始复杂任务前，制定执行计划
//   - 多步骤任务的中途，更新剩余步骤
//   - 任务完成后，归档执行记录
//   - 与其他工具配合（如SequentialThinking），形成完整的任务管理
//
// 典型使用流程:
//   1. Agent分析用户需求，拆解为多个步骤
//   2. 调用TodoWrite创建计划，记录任务和步骤
//   3. 按步骤执行，每完成一步可更新计划（移除已完成步骤）
//   4. 所有步骤完成后，标记任务完成
//
// 与SequentialThinking的区别:
//   - SequentialThinking: 记录思考过程，支持分支，用于推理
//   - TodoWrite: 记录行动计划，线性步骤，用于任务管理
//
// 示例:
...
//			  "task":         "完成数据分析报告",
//			  "steps":        []string{"从数据库提取销售数据", "清洗数据...", ...},
//			  "steps_json":   "[\"从数据库提取销售数据\",\"清洗数据...\",...]",
//			  "total_steps":  5,
//			  "plan_created": true,
//			  "display_type": "plan",
//		  },
