...
// 本插件处理 CHUNK_MERGE 事件，执行知识片段的物理合并、FAQ 补全及上下文动态扩展。
//
// 它在搜索流程的最后阶段运行，将零散的检索结果片段（Chunks）进行“深加工”，使其变成大模型（LLM）更容易理解和利用的高质量上下文。
// 简单来说，它的任务不仅仅是把搜到的句子堆在一起，而是要消除重叠、补充图片、补全问答、并扩充过短的片段，
// 解决了向量搜索中常见的片段重叠、上下文缺失以及多媒体信息丢失等工程痛点。
//
// 该插件主要执行以下四个关键任务：
//	- 物理合并 (Physical Merge)：将同一文档中位置重叠或相邻的片段缝合起来，去除重复内容。
//	- 图片元数据整合 (Image Merge)：将分散在不同片段中的图片信息合并，确保引用的完整性。
//	- FAQ 内容补全 (FAQ Populate)：将仅包含 ID 的 FAQ 片段还原为完整的“问-答”文本。
//	- 短上下文扩展 (Context Expand)：对于字数过少的片段，自动向前后寻找邻居节点进行扩展，提供足够的语境。
//
// 核心功能详解
//
