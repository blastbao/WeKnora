...
//
// 功能说明:
//   - 解析异步任务负载，获取租户 ID、分块 ID 和模型 ID
//   - 初始化日志上下文（请求 ID、分块标识、租户信息）
//   - 获取分块内容和所属知识库配置
//   - 根据知识库的提取配置构建提示词模板
//   - 调用大语言模型从文本中提取实体（节点）和关系（边）
//   - 将提取的图谱数据关联到对应分块，存入图数据库
//
// 参数:
//   - ctx: 上下文，用于控制执行超时和传递日志追踪信息
//   - t: 异步任务对象，包含序列化的 ExtractChunkPayload 数据
//
// 返回值:
//   - error: 处理过程中的错误，nil 表示处理成功
//
// 执行流程:
//   1. 反序列化任务负载 → 2. 初始化日志上下文 → 3. 获取分块数据
//   4. 获取知识库配置 → 5. 获取聊天模型 → 6. 构建提取模板
//   7. 执行图谱提取 → 8. 重新获取分块（确认有效性）
//   9. 关联分块 ID 到节点 → 10. 存入图数据库
//
// 图谱提取配置:
//   - 使用知识库的 ExtractConfig 作为提取模板
//   - 包含预定义的实体标签（Tags）、示例文本（Text）
//   - 包含节点类型（Nodes）和关系类型（Relations）定义
//   - 通过示例引导模型按指定格式输出结构化图谱数据
//
// 注意事项:
//   - 任务执行前需确保 Neo4j 已启用（环境变量 NEO4J_ENABLE=true）
//   - 分块在提取过程中可能被删除，二次获取失败时优雅忽略
//   - 图谱提取为计算密集型操作，依赖外部 LLM 服务可用性
//   - 提取模板通过知识库配置动态构建，支持不同领域的实体关系定义
...
// 执行流程:
//   第1步: 解析任务
//   • 从消息队列获取任务数据（JSON格式），提取：租户ID、文件ID、AI模型配置
//   • 设置日志追踪（方便排查问题）
//
//   第2步: 准备资源 (prepareResources)
//   • 找到要处理的文件（Excel/CSV）
//   • 检查文件类型（不是表格就报错）
//   • 加载两个AI模型：
//     - 聊天模型：负责写摘要（描述表格）
...
//   • 获取搜索引擎实例
//
//   第3步: 处理表格数据 (processTableData)
//    3.1 加载数据 ──► 把Excel/CSV导入DuckDB（内存数据库）
//    3.2 抽取样本 ──► 取前10行数据作为参考
//    3.3 AI生成摘要 ──► 问AI："这个表格是干嘛的？"，生成：表格主题、字段含义、业务场景等
//    3.4 AI生成列描述 ──► 问AI："每一列是什么意思？"，生成：字段类型、业务用途、数据特征等
//    3.5 构建Chunks ──► 创建两个文档块：表格摘要（整体描述），列描述（字段详情）
//
//   第4步: 建立索引 (indexToVectorDB)
//   • 把两个描述文本转成向量（Embedding）
//   • 存入向量数据库（Milvus/Elasticsearch等）
//   • 更新状态为"已索引"
//   • 如果这一步失败 ──► 执行cleanupOnFailure()
//                      • 删除已创建的文档块
//                      • 删除向量数据库中的索引
//                      • 标记文件处理失败
